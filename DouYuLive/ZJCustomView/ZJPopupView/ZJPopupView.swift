...
class ZJPopupView: UIView, ZJPopupViewDelegate {
    ...
    init(size: CGSize, customView: ZJPopupBaseView? = nil, style: ZJPopupAnimationStyle) {
        ...
        // 背景层点击事件
        // 自定义视图点击事件，默认不做处理
        let customTap = UITapGestureRecognizer(target: self, action: #selector(zj_customTap))
        self.customView.addGestureRecognizer(customTap)
        
        // suctomView的点击事件，隐藏弹框
...
extension ZJPopupView {
    @objc private func zj_bgViewTap() {
        guard isBGClickAction else { return }
        
        self.zj_dissmissPopView()
    }
    
    @objc private func zj_customTap() {
...
extension ZJPopupView {
    ...
    func zj_showPopView() {
        ...
        case .ZJPopTransition?:
            self.customView.transform = self.customView.transform.concatenating(CGAffineTransform(translationX: 0, y: -(kScreenH - self.customViewSize.height)/2))
            UIView.animate(withDuration: durationTime) {
                self.alpha = 1.0
                self.customView.transform = self.customView.transform.concatenating(CGAffineTransform(translationX: 0, y: (kScreenH - self.customViewSize.height)/2+50))
